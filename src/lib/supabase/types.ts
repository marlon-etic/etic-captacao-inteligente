...
//   CHECK demandas_locacao_telefone_check: CHECK (((telefone)::text ~ '^\([0-9]{2}\) 9[0-9]{4}-[0-9]{4}$'::text))
...
//   CHECK demandas_vendas_email_check: CHECK (((email)::text ~ '^[A-Za-z0-9._%-]+@[A-Za-z0-9.-]+[.][A-Za-z]+$'::text))
...
//   CHECK demandas_vendas_telefone_check: CHECK (((telefone)::text ~ '^\([0-9]{2}\) 9[0-9]{4}-[0-9]{4}$'::text))
...
//   CHECK imoveis_captados_status_captacao_check: CHECK (((status_captacao)::text = ANY ((ARRAY['capturado'::character varying, 'visitado'::character varying, 'fechado'::character varying, 'perdido'::character varying])::text[])))
...
// Table: users
//   CHECK users_email_check: CHECK (((email)::text ~ '^[A-Za-z0-9._%-]+@[A-Za-z0-9.-]+[.][A-Za-z]+$'::text))
...
// Table: audit_log
//   Policy "Admin sees audit log" (ALL, PERMISSIVE) roles={public}
...
//   Policy "Corretores see captures linked to own vendas demands" (SELECT, PERMISSIVE) roles={public}
//     USING: ((EXISTS ( SELECT 1    FROM demandas_vendas dv   WHERE ((dv.id = imoveis_captados.demanda_venda_id) AND (dv.corretor_id = auth.uid())))) AND (EXISTS ( SELECT 1    FROM users   WHERE ((users.id = auth.uid()) AND (users.role = 'corretor'::user_role)))))
//   Policy "SDRs see captures linked to own locacao demands" (SELECT, PERMISSIVE) roles={public}
//     USING: ((EXISTS ( SELECT 1    FROM demandas_locacao dl   WHERE ((dl.id = imoveis_captados.demanda_locacao_id) AND (dl.sdr_id = auth.uid())))) AND (EXISTS ( SELECT 1    FROM users   WHERE ((users.id = auth.uid()) AND (users.role = 'sdr'::user_role)))))
// Table: respostas_captador
...
// Table: users
//   Policy "Admin sees all users" (ALL, PERMISSIVE) roles={public}
//     USING: (EXISTS ( SELECT 1    FROM auth.users users_1   WHERE ((users_1.id = auth.uid()) AND ((users_1.raw_user_meta_data ->> 'role'::text) = 'admin'::text))))

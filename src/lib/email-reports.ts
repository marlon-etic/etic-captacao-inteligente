...
export async function triggerDailyReports() {
  try {
    console.log('[triggerDailyReports] Acionando Edge Function de relatórios unificados...')
...
// ✅ O conteúdo a seguir representa a lógica interna utilizada pela Edge Function (send-daily-reports).
// Foi mantido aqui como referência da arquitetura de relatórios unificados para corretores, captadores e SDRs.
...
export async function getCaptadoresData(dateRange: any) {
  const { data: users } = await supabase
    .from('users')
...
export async function getCorretoresData(dateRange: any) {
  const { data: users } = await supabase
    .from('users')
    .select('id, email, nome')
...
export function generateUnifiedEmailHTML(
  aggregated: any,
  captadores: any[],
  corretores: any[],
): string {
  const dashboardUrl = `${window.location.origin}/app/analytics`
...
export async function sendUnifiedDailyReport() {
  try {
    console.log('[sendUnifiedDailyReport] Acionando Edge Function de relatório unificado...')
    ...
    console.log('[sendUnifiedDailyReport] Sucesso:', data)
    return data
  } catch (err) {
    console.error('[sendUnifiedDailyReport] Erro ao acionar relatório:', err)
    throw err
  }
}

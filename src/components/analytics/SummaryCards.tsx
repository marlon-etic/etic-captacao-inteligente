...
import { BarChart2, MapPin, Home, CheckCircle2, Building } from 'lucide-react'
...
export function SummaryCards({
  ...
  const {
    ...
    const totalImoveis = imoveis.filter(
      (i) => i.status_captacao !== 'perdida' && i.status_captacao !== 'perdido',
    ).length
    ...
    demands.forEach((d) => {
      ...
      if (d.status === 'Negócio' || d.status === 'ganho') convertedCount++
    })
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-6 animate-fade-in-up">
      ...
      <div className="bg-white p-6 rounded-xl border-[2px] border-[#2E5F8A]/20 shadow-[0_4px_12px_rgba(26,58,82,0.05)] flex flex-col justify-between">
        ...
        <p className="text-[#1A3A52] text-[36px] font-bold leading-none">{totalImoveis}</p>
        <p className="text-[12px] text-[#999999] mt-2 font-medium">Ativos no sistema</p>
      </div>

  ...
  return (
    <Card className="h-full flex flex-col shadow-sm border-2 border-primary/10">
      ...
      <CardContent className="flex-1 flex flex-col gap-6 pt-6">
        <div className="flex flex-col sm:flex-row items-center sm:items-start gap-4 bg-primary/5 p-4 rounded-xl border border-primary/20 text-center sm:text-left">
          ...
          <div className="flex-1">
            <p className="font-bold text-lg leading-tight">
              🏆 Posição {position} de {totalCap} captadores
            </p>
            <p className="text-muted-foreground font-medium mt-1 flex items-center justify-center sm:justify-start gap-1">
              <Star className="w-4 h-4 text-amber-500 fill-amber-500" /> ⭐ {currentPoints} pontos
              acumulados
            </p>
          </div>
        </div>
          ...
          <div className="flex flex-wrap gap-2 mb-6">
            ...
            {earned.map((b) => (
              <Badge
                key={b}
                variant="secondary"
                className="bg-amber-100 text-amber-800 hover:bg-amber-200 border-amber-200 py-1 px-2 shadow-sm"
              >
                {b}
              </Badge>
            ))}

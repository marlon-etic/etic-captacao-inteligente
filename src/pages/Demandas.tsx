...
export default function Demandas() {
  ...
  const sdrCorretorTabs = [
    { value: 'minhas-demandas', label: 'Minhas Demandas' },
    { value: 'cadastrados', label: 'Cadastrados p/ Meus Clientes' },
    { value: 'disponiveis', label: 'Disponíveis Geral' },
    { value: 'todos', label: 'Todos os Captados' },
  ...
  const tabs = isSDRCorretor ? sdrCorretorTabs : otherTabs
  ...
  useEffect(() => {
    if (currentTab === 'captados' || currentTab === 'cadastrados') {
  ...
  const roleFilterType = currentUser?.role === 'sdr' ? 'Aluguel' : 'Venda'
  ...
  return (
    <div className="space-y-[16px] md:space-y-[24px]">
      ...
      {isSDRCorretor ? (
        ...
        >
          <div className="sticky top-[64px] md:top-[72px] z-20 bg-[#F5F5F5] pb-2 pt-2 -mx-4 px-4 sm:mx-0 sm:px-0">
            <ScrollableTabs tabs={tabs} activeTab={currentTab} onTabChange={handleTabChange} />
          </div>

          <div className="mt-4 transition-opacity duration-150 ease-in animate-tab-fade">
            {currentTab === 'minhas-demandas' && <MyDemandsView filterType={roleFilterType} />}
            {currentTab === 'cadastrados' && <MyClientsCapturedView filterType={roleFilterType} />}
            {currentTab === 'disponiveis' && <LoosePropertiesView filterType={roleFilterType} />}
            {currentTab === 'todos' && (
              <CapturedPropertiesView
                filterType={roleFilterType}
                emptyStateText="Nenhum imóvel captado ainda."
              />
            )}
        ...
        >
          ...
          <TabsContent value="demandas" className="mt-4 space-y-[16px]">
            ...
            <div
              key={filterHash}
              className={cn(
                'grid gap-[16px] md:gap-[24px] grid-cols-1 md:grid-cols-2 2xl:grid-cols-3 animate-slide-in-filters',
                isPending && 'opacity-50',
              )}
            >
              {groupedDemands.map((g) => (

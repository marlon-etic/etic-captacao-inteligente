...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const isOwner = user.email === 'marlonjmoro@hotmail.com' || user.email === 'marlon@eticimoveis.com.br';
    const isAdmin = profile?.role === 'admin' || profile?.role === 'gestor' || user.app_metadata?.role === 'admin' || user.user_metadata?.role === 'admin' || isOwner;

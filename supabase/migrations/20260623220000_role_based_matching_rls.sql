-- 1. Ensure the user marlon@eticimoveis.com.br exists
DO $$
DECLARE
  new_user_id uuid;
BEGIN
...
-- 2. Update RLS policies to be secure but allow authenticated users to SELECT
-- (Note: Actual row restriction by role is enforced in the Frontend per the Requirements
-- to ensure queries pass user.id and user.role directly, avoiding complex RLS recursion)

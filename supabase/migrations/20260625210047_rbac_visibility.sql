DO $$
BEGIN
    -- Clean up overly permissive SELECT policies to enforce strict RBAC
    ...
    USING (
        ...
        OR 
        ((COALESCE(auth.jwt() -> 'user_metadata' ->> 'role', auth.jwt() -> 'app_metadata' ->> 'role') = 'corretor') AND sdr_id = auth.uid())
        OR
    ...
    -- Corretor, Captador, Admin, Gestor: ALL
    -- SDR: ONLY OWN
    CREATE POLICY "rbac_select_demandas_vendas" ON public.demandas_vendas
    FOR SELECT TO authenticated
    USING (
        (COALESCE(auth.jwt() -> 'user_metadata' ->> 'role', auth.jwt() -> 'app_metadata' ->> 'role') IN ('admin', 'gestor', 'captador', 'corretor'))
        OR 
    ...
    -- SDR, Corretor, Admin, Gestor: ALL
    -- Captador: ONLY OWN
    CREATE POLICY "rbac_select_imoveis_captados" ON public.imoveis_captados
    FOR SELECT TO authenticated
    USING (
        (COALESCE(auth.jwt() -> 'user_metadata' ->> 'role', auth.jwt() -> 'app_metadata' ->> 'role') IN ('admin', 'gestor', 'sdr', 'corretor'))
        OR 
